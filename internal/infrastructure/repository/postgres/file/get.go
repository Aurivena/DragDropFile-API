...
func (r *File) ByID(id string) (*entity.File, error) {
	var out entity.File
	err := r.db.Get(&out, `SELECT S.file_id,name,mime_type,FP.session,password,date_deleted,count_download,description FROM "FilePayload"
			INNER JOIN public."File_Parameters" FP on "FilePayload".id = FP.file_id
			INNER JOIN "SessionID" S ON S.file_id = "FilePayload".id
			WHERE "FilePayload".file_id = $1`, id)
	if err != nil {
		return nil, err
	}
...
func (r *File) ZipMetaBySession(sessionID string) (*entity.File, error) {
	var out entity.File
	err := r.db.Get(&out, `SELECT "FilePayload".id,"FilePayload".file_id, name FROM "FilePayload" 
                INNER JOIN "SessionID" ON "SessionID".file_id = "FilePayload".id
    			WHERE "SessionID".session = $1 AND "FilePayload".name LIKE '%.zip'`, sessionID)
	if err != nil {
...
func (r *File) IdFilesBySession(sessionID string) ([]string, error) {
	...
	err := r.db.Select(&out, `SELECT F.file_id FROM "SessionID"
               INNER JOIN public."FilePayload" F on F.id = "SessionID".file_id
...
func (r *File) DataFile(id string) (*entity.FileData, error) {
	...
	err := r.db.Get(&out, `SELECT (password IS NOT NULL AND password != '') AS password,date_deleted,count_download,description
					...
					INNER JOIN public."FilePayload" F on F.id = "File_Parameters".file_id
